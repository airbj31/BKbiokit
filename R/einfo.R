...
einfo <- function(db="") {
  ...
  if(db=="") {
    ...
    Output$Link  <- do.call(cbind,lapply(paste0(".//LinkList/Link/",LinkColumns),function(x) BKbiokit::xml2vector(search_result,x))) |> as.data.frame() |> tibble::as_tibble()
    Output$Field <- do.call(cbind,lapply(paste0(".//FieldList/Field/",FieldColumns),function(x) BKbiokit::xml2vector(search_result,x))) |> as.data.frame() |> tibble::as_tibble()
